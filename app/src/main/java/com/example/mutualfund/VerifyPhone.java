...
public class VerifyPhone extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		progressDialog = new ProgressDialog ( VerifyPhone.this );
	...
	private void sendVerificationCode ( String phoneNo ) {
		PhoneAuthProvider.getInstance ().verifyPhoneNumber ( phoneNo, 90, TimeUnit.SECONDS, VerifyPhone.this, mCallBack );
	}
	...
	private final PhoneAuthProvider.OnVerificationStateChangedCallbacks mCallBack = new PhoneAuthProvider.OnVerificationStateChangedCallbacks ( ) {
		...
		public void onVerificationFailed(@NonNull FirebaseException e) {
			...
			FancyToast.makeText ( VerifyPhone.this, e.getMessage (), FancyToast.LENGTH_LONG, FancyToast.ERROR, false ).show ();
		}
	...
	private void linkCredential(PhoneAuthCredential phoneAuthCredential) {
		Objects.requireNonNull ( firebaseAuth.getCurrentUser ( ) ).linkWithCredential ( phoneAuthCredential ).addOnCompleteListener ( VerifyPhone.this, task -> {
			if (task.isSuccessful ()) {
				...
				progressDialog.dismiss();
				FancyToast.makeText ( VerifyPhone.this, Objects.requireNonNull ( task.getException ( ) ).getMessage (), FancyToast.LENGTH_LONG, FancyToast.ERROR, false ).show ();
			}
		} );
	}
